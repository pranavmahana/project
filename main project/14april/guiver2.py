    ...
    def plot_psd(self):
        if self.time_data is not None and self.g_level_data is not None:
            if self.selected_range:
                ...
                selected_time = self.time_data[start:end]
                selected_data = self.g_level_data[start:end, :]
            else:
                selected_time = self.time_data
                selected_data = self.g_level_data
                
            fs = self.sampling_frequency.get()
            for i in range(3):
                f, psd = welch(selected_data[:, i], fs=fs)
                self.psd_ax[i].clear()
