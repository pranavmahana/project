...
class PSDPlotter(tk.Tk):
    def __init__(self):
        ...
        self.glevel_tab = ttk.Frame(self.tab_control)
        self.plot_tab = ttk.Frame(self.tab_control)

        self.tab_control.add(self.input_tab, text="Input")
        self.tab_control.add(self.glevel_tab, text="G-Levels")
        self.tab_control.add(self.plot_tab, text="PSD")
    ...
    def plot_glevels(self):
        try:
            ...
            ax.plot(np.arange(len(self.data)) / fs, np.sqrt(np.sum(self.data**2, axis=1)) / sensitivity, color='blue')
            ax.set_title('G-Levels')
            ax.set_xlabel('Time (s)')
            ax.set_ylabel('G-Levels')

            canvas = FigureCanvasTkAgg(fig, master=self.glevel_tab)
            canvas.draw()
