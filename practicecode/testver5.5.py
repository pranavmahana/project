...
class PSDPlotterApp:
    def __init__(self, root):
        ...
        self.fig_g_levels = None
        self.ax_g_levels = None
        self.canvas_g_levels = None
        self.fig_psd = None
        self.ax_psd = None
    ...
    def create_widgets(self):
        ...
        self.fig_g_levels, self.ax_g_levels = plt.subplots(figsize=(8, 6))
        self.canvas_g_levels = FigureCanvasTkAgg(self.fig_g_levels, master=self.root)
        self.canvas_g_levels.get_tk_widget().pack()
        ...
        self.canvas_psd = FigureCanvasTkAgg(self.fig_psd, master=self.root)
        ...
        # Span Selector
        self.span_selector = SpanSelector(self.ax_psd, self.onselect, 'horizontal', useblit=True)
    ...
    def calculate_psd(self, data, fs, n, nperseg=None, noverlap=None, nfft=None, window_type='hann'):
        f, Pxx = welch(data, fs, nperseg=nperseg, noverlap=noverlap, nfft=nfft, window=window_type)

        Pxx = 10 * np.log10(np.maximum(Pxx, np.finfo(float).tiny))  # Assuming sensitivity is not provided
