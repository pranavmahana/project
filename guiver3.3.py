...
class PSDPlotterApp:
    def __init__(self, root):
        ...
        self.glevel_range_label.grid(row=7, column=1, pady=5, padx=10, sticky="w")

        self.canvas_frame = ttk.Frame(root)
        self.canvas_frame.grid(row=8, column=0, columnspan=3, pady=5, padx=10, sticky="nsew")
        ...
        self.plot_psd_button.grid(row=9, column=0, pady=10, padx=10, sticky="w")

        self.window_type_label = ttk.Label(root, text="Window Type (default: 'hann'):")
        self.window_type_label.grid(row=9, column=1, pady=5, padx=10, sticky="w")

        self.window_type_entry = ttk.Entry(root)
        self.window_type_entry.grid(row=9, column=2, pady=5, padx=10, sticky="w")
    ...
    def plot_psd(self):
        ...
        if data is not None:
            for i in range(3):
                ...
                x_selected = x[self.glevel_start:self.glevel_end + 1]

                # Plot PSD for the selected range
                plot_psd(self.axs[1, i], x_selected, fs, nperseg, noverlap, nfft, get_window(window_type, nperseg), f'PSD Plot {i + 1}')
    ...
    def on_glevel_click(self, event):
        if event.inaxes in self.axs[0]:
            ...
            self.glevel_start = x_click
            self.glevel_end = x_click + 100  # Adjust the range size as needed

            # Update the entry box
            self.glevel_range_label.config(text=f"Selected G-level Range: {self.glevel_start} - {self.glevel_end}")
            self.canvas_widget.draw()
